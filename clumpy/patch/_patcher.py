...
class Patcher():
    ...
    def allocate(self,
        ...
        while len(J_allocated) < area:
            ...
            if self.avoid_aggregation and (np.sum((vi_neighbors == initial_state) * (vf_neighbors == final_state)) > 0):
            ...
            id_j_neighbors_to_keep = np.arange(j_neighbors.size)[(vi_neighbors == initial_state) * (vf_neighbors == initial_state)]
        ...
        if self.avoid_aggregation:
            ...
            if np.any((vi_neighbors == initial_state) & (vf_neighbors == final_state)):
                # print('aggrégation finale')
                return(0, J_allocated)
                
        # on peut procéder à l'allocation réelle
        lul.flat[J_allocated] = final_state
